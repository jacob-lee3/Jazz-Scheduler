...
if (st.session_state.tab_state == 'KickstartModel' and "validation_error" not in st.session_state):
    ...
    with tab_solve:
        ...
        if "thank_you" in st.session_state:
            st.success('Thank you for checking out the program! Note that this version of the program does not include solver functionality, and exists' \
                'purely to showcase the UI and model-building features. To use the full program, install cbc and the full project files.')
            if st.button('CONTINUE'):
                st.session_state.ready_for_assignments = 'Unsolved'
                st.session_state.confirm_schedule_box = False
                del st.session_state.thank_you
                st.rerun()
